...
export const getPaciente = async(req, res) => {
    try{
        ...
        const [result] = await pool.query('SELECT pacienteid, nombre, apellido, ci, fechaNac, sexo, sangre,b.barrioId, b.descripcion AS barrio, c.ciudadId, c.descripcion AS ciudad, telefono, direccion, fechaReg, ultTurno, usuarioId  FROM pacientes p JOIN ciudades c ON (p.ciudadid = c.ciudadid) JOIN barrios b ON (p.barrioid = b.barrioid) WHERE pacienteid = ?', [id]);
        if(result.length == 0){
