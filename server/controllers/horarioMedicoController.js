...
export const getHorarioMedico = async(req, res) => {
    try{
        ...
        const [result] = await pool.query('SELECT horariomedicoid, nombre, apellido, e.descripcion AS especialidad, intervConsulta, dia, desde, hasta, h.estado FROM horariosmedicos hm JOIN horarios h ON (hm.horarioId = h.horarioId) JOIN medicos m ON (m.medicoId = hm.medicoId) JOIN especialidades e ON (m.especialidadId = e.especialidadId) WHERE horariomedicoid = ?', [id]);
        if(result.length == 0){
